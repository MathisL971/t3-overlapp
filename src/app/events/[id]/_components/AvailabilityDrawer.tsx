...
export const constructSlotGrid = (
  ...
  for (let i = startHour; i < endHour; i++) {
    ...
    const existingSlot1 = existingAvailabilities.find(
      (availability) => availability.startTime === `${i < 10 ? "0" + i : i
        }:00:00`,
    );
...
export default function AvailabilityDrawer(
  ...
  const firstHalf = constructSlotGrid(startHour, middleHour, availabilities.filter(
    (availability) => {
      const startTime = availability.startTime.split(":")[0];
      return parseInt(startTime!) < middleHour
    },
  ));
  const secondHalf = constructSlotGrid(middleHour, endHour, availabilities.filter(
    (availability) => {
      const startTime = availability.startTime.split(":")[0];
      return parseInt(startTime!) >= middleHour
    },
  ...
  return (
    <Drawer key={day.id}>
      <DrawerTrigger className="rounded-md border py-2 px-3 flex flex-row justify-between items-center">
        <h6 className="m-0">{title}</h6>
        <Badge variant={"secondary"}>{
          firstHalf.filter((slot) => slot.savedSlotId).length + secondHalf.filter((slot) => slot.savedSlotId).length
        }</Badge>
      </DrawerTrigger>
      <DrawerContent>
        <DrawerHeader>
